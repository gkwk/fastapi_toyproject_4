...
celery_beat_app = Celery(__name__, broker=get_settings().CELERY_BROKER_PATH_URL)
...
celery_beat_app.conf.task_routes = {
...
@celery_beat_app.task(name=get_settings().CELERY_BEAT_TEST_TASK_NAME, queue=get_settings().CELERY_BEAT_QUEUE_NAME)
@AsyncioHandler.async_to_sync
async def oscillatort():
